...
class Tools():
    ...
    def select_chatList(main_window:WindowSpecification)->(ListItemWrapper|None):
        ...
        # 版本适配: 微信 4.1 不同小版本中, 消息项控件类型不同
        # - 早期版本(4.1.6 初期等)使用 CheckBox
        # - 较新版本(4.1.6.46, 4.1.9.x 等)使用 ListItem
        if chatList.children(control_type='ListItem'):
            selected=[item for item in chatList.children(control_type='ListItem')]
        elif chatList.children(control_type='CheckBox'):
...
class Navigator():
    ...
    def open_weixin(is_maximize:bool=None,window_size:tuple=None)->(WindowSpecification|None):
        ...
        Args:
            ...
            window_size:微信主界面大小(宽,高),默认None按屏幕比例(宽50%高100%),可GlobalConfig.window_size=(width,height)全局控制
        '''
        ...
        if main_window.class_name()=='mmui::MainWindow':
            ...
            #window_size为None时,默认按屏幕比例:宽50%,高100%(占满可用屏幕高度),窗口靠左贴边
            if window_size is None:
                window_width,window_height=int(screen_width*0.5),screen_height
                new_left=work_area[0]#靠左贴边
            else:
                window_width,window_height=window_size[0],window_size[1]
                new_left=work_area[0]+(screen_width-window_width)//2#自定义尺寸水平居中
            win32gui.SetWindowPos(hwnd,win32con.HWND_TOPMOST,
            0, 0,window_width,window_height,win32con.SWP_NOMOVE)
            new_top=work_area[1]+(screen_height-window_height)//2
            if screen_width!=window_width:
                #移动窗口(默认靠左贴边并占满可用高度;自定义尺寸时水平居中)
