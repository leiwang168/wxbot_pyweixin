...
#拉黑/删除提示关键词(简体中文):发出的消息被拒收时聊天流里出现的系统反馈
BLACK_HINTS = ('被对方拒收',)        #对方已把你拉黑
DELETE_HINTS = ('开启了朋友验证',)   #对方已把你删除

#本地聊天文件目录缓存(where_* 系列每次都遍历进程内存映射,缓存一次即可)
_media_dirs_cache = {}

def get_media_dirs():
    '''惰性获取并缓存 msg 媒体目录。微信 4.1.9.x 图片/视频在 attach 子目录下。'''
    if not _media_dirs_cache:
        msg_folder = Tools.where_msg_folder()
        _media_dirs_cache['file'] = os.path.join(msg_folder, 'file')
        # 微信 4.1.9.x: 图片/视频移至 msg/attach/{hash}/{月份}/Img(Video)/
        # 兼容旧版: msg/image/, msg/video/
        img_new = os.path.join(msg_folder, 'attach')
        img_old = os.path.join(msg_folder, 'image')
        _media_dirs_cache['image'] = img_new if os.path.isdir(img_new) else img_old
        vid_new = os.path.join(msg_folder, 'attach')
        vid_old = os.path.join(msg_folder, 'video')
        _media_dirs_cache['video'] = vid_new if os.path.isdir(vid_new) else vid_old
    return _media_dirs_cache


def classify_message(item):
    """识别一条聊天消息,返回(展示文本,消息类型,本地路径)。
    覆盖文本/语音/图片/视频/动画表情/文件/链接/小程序/视频号/红包/转账/名片/通话/系统消息等;
    文件返回本地完整路径,图片/视频返回缓存目录(精确文件名需 Messages.save_media 提取);
    被拉黑/删除也能识别。判断依据:控件 class_name + window_text() 内的类型标签。
    """
    text = item.window_text() or ''
    cls = item.class_name() or ''
    #优先识别被拉黑/删除(发出的消息被拒收时的系统反馈)
    if any(k in text for k in BLACK_HINTS):
        return text, '被拉黑', None
    if any(k in text for k in DELETE_HINTS):
        return text, '被删除', None
    #语音消息: '语音3"秒未播放'/'语音3"秒已播放'/'语音3"秒<转文字>'
    if Regex_Patterns.Audio_pattern.search(text):
        return text, '语音', None
    if cls == 'mmui::ChatTextItemView':
        return text, '文本', None
    if cls == 'mmui::ChatPersonalCardItemView':
        return text, '好友名片', None
    if cls == 'mmui::ChatBubbleReferItemView':
        dirs = get_media_dirs()
        if Special_Labels.Image in text:    return text, '图片', dirs['image']
        if Special_Labels.Video in text:    return text, '视频', dirs['video']
        if Special_Labels.Emoji in text:    return text, '动画表情', None
        return text, '引用消息', None
    if cls == 'mmui::ChatBubbleItemView':
        if Special_Labels.File in text:
            m = Regex_Patterns.File_pattern.search(text)
            filename = m.group(1) if m else ''
            path = os.path.join(get_media_dirs()['file'], time.strftime('%Y-%m'), filename) if filename else None
            return text, '文件', path
        if Special_Labels.Link in text:        return text, '链接', None
        if Special_Labels.MiniProgram in text: return text, '小程序', None
        if Special_Labels.Channels in text:    return text, '视频号', None
        if Special_Labels.RedPacket in text:   return text, '微信红包', None
        if Special_Labels.Transfer in text:    return text, '微信转账', None
        if Special_Labels.ChatHistory in text: return text, '聊天记录', None
        if Special_Labels.VoiceCall in text:   return text, '语音通话', None
        if Special_Labels.VideoCall in text:   return text, '视频通话', None
        return text, '其他', None
    if cls == 'mmui::ChatItemView':
        return text, '系统消息', None
    return text, '未知', None
