...
class FriendSettings():
    ...
    def add_new_friend(number:str,greetings:str=None,remark:str=None,chat_only:bool=False,is_maximize:bool=None,close_weixin:bool=None):
        ...
        Args:
            ...
            close_weixin:任务结束后是否关闭微信，默认关闭
        Raises:
            NoSuchFriendError:搜不到该微信号/手机号对应的用户(号码不存在或输错),此时不会发出申请
            ValueError:对方已经是你的好友(资料页无"添加到通讯录"按钮),无需重复添加
        '''
        if is_maximize is None:
            is_maximize=GlobalConfig.is_maximize
        if close_weixin is None:
...
class Monitor():
    ...
    def listen_on_chat(dialog_window:WindowSpecification,duration:str,groupMembers:list=[],save_file:bool=False,save_media:bool=False,target_folder:str=None,close_dialog_window:bool=True)->dict:
        ...
        # 优先检测 ListItem, 回退到 CheckBox, 兼容所有版本
        if chatList.children(control_type='ListItem'):
            _msg_control_type='ListItem'
        elif chatList.children(control_type='CheckBox'):
            _msg_control_type='CheckBox'
        else:
            _msg_control_type='ListItem'#默认 ListItem, 运行时若找不到会返回空列表
        visibile_items=chatList.children(control_type=_msg_control_type)
