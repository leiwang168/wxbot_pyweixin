...
class globalConfig:
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            ...
            cls._instance._window_size=None#None表示按屏幕比例(默认宽50%高100%)
    ...
    def window_size(self):
        '''微信主界面大小设定(宽,高),None表示按屏幕比例(默认宽50%高100%)'''
        return self._window_size

    @window_size.setter
    def window_size(self,value):
        if value is not None and not isinstance(value,tuple):
            raise TypeError(f"window_size必须是tuple类型或None,但传入了{type(value)}:{value}")
